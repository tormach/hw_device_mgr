...
from ...cia_301.tests.test_device_read_update_write import (
    TestCiA301DeviceRUW as _TestCiA301DeviceRUW,
)


class TestCiA402DeviceRUW(_TestCiA301DeviceRUW, BaseCiA402TestClass):
...
class TestCiA402DeviceRUWHMTimeout(TestCiA402DeviceRUW):
    def test_read_update_write(self, obj, mock_time):
        # Test HM mode homing operation timeout
        #
...
class TestCiA402DeviceRUWPPTimeout(TestCiA402DeviceRUW):
    def test_read_update_write(self, obj, mock_time):
        # Test PP mode move timeout
        #
        # Only applies to CiA402 devices
        if not hasattr(obj, "MODE_CSP"):
            return
