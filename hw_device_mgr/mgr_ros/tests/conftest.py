...
def mock_rclpy(request):
    ...
    def get_parameter(name):
        res = request.instance.rosparam_decls[name]
        print(f"  mock_rclpy:  node.get_parameter({name}) = {res}")
        return res

    def get_parameters_by_prefix(prefix):
        return {
            k: v
            for k, v in request.instance.rosparam_decls
    ...
    def create_node(node_name, **kwargs):
        ...
        node = request.instance.node = MagicMock(
            ...
            get_parameter=MagicMock(side_effect=get_parameter),
            get_parameters_by_prefix=MagicMock(
                side_effoct=get_parameters_by_prefix
            ),
